...
@interface ZGObjectSerializer ()

@end

@implementation ZGObjectSerializer

{
    ZGObjectSerializerConfig *_configuration;
    ZGObjectIdentityProvider *_objectIdentityProvider;
}
...
- (NSDictionary *)serializedObjectsWithRootObject:(id)rootObject
{
    ...
    ZGObjectSerializerContext *context = [[ZGObjectSerializerContext alloc] initWithRootObject:rootObject];
...
- (void)visitObject:(NSObject *)object withContext:(ZGObjectSerializerContext *)context
{
    ...
    ZGClassDescription *classDescription = [self classDescriptionForObject:object];
    if (classDescription) {
        for (ZGPropertyDescription *propertyDescription in [classDescription propertyDescriptions]) {
    ...
    if (classDescription && [[classDescription delegateInfos] count] > 0 && [object respondsToSelector:delegateSelector]) {
        ...
        for (ZGDelegateInfo *delegateInfo in [classDescription delegateInfos]) {
...
- (NSArray *)allValuesForType:(NSString *)typeName
{
    ...
    ZGTypeDescription *typeDescription = [_configuration typeWithName:typeName];
    if ([typeDescription isKindOfClass:[ZGEnumDescription class]]) {
        ZGEnumDescription *enumDescription = (ZGEnumDescription *)typeDescription;
...
- (NSArray *)parameterVariationsForPropertySelector:(ZGPropertySelectorDescription *)selectorDescription
{
    ...
    if ([selectorDescription.parameters count] > 0) {
        ZGPropertySelectorParameterDescription *parameterDescription = (selectorDescription.parameters)[0];
...
- (id)instanceVariableValueForObject:(id)object propertyDescription:(ZGPropertyDescription *)propertyDescription
{
    NSParameterAssert(object != nil);
...
- (NSInvocation *)invocationForObject:(id)object withSelectorDescription:(ZGPropertySelectorDescription *)selectorDescription
{
...
- (id)propertyValue:(id)propertyValue propertyDescription:(ZGPropertyDescription *)propertyDescription context:(ZGObjectSerializerContext *)context
{
...
- (id)propertyValueForObject:(NSObject *)object withPropertyDescription:(ZGPropertyDescription *)propertyDescription context:(ZGObjectSerializerContext *)context
{
    NSMutableArray *values = [[NSMutableArray alloc] init];

    ZGPropertySelectorDescription *selectorDescription = propertyDescription.getSelectorDescription;
    ...
    else if (propertyDescription.useInstanceVariableAccess)
    {
        ...
        if (invocation) {
            ...
            for (NSArray *parameters in parameterVariations) {
                [invocation zg_setArgumentsFromArray:parameters];
                [invocation invokeWithTarget:object];

                id returnValue = [invocation zg_returnValue];
...
- (ZGClassDescription *)classDescriptionForObject:(NSObject *)object
{
    NSParameterAssert(object != nil);
    ...
    while (aClass != nil)
    {
        ZGClassDescription *classDescription = [_configuration classWithName:NSStringFromClass(aClass)];
        if (classDescription) {
