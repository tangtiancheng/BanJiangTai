...
@implementation ZGUIControlBinding
...
+ (ZGEventBinding *)bindingWithJSONObject:(NSDictionary *)object
{
    ...
    if (![path isKindOfClass:[NSString class]] || [path length] < 1) {
        ZhugeDebug(@"must supply a view path to bind by");
        return nil;
    }
    ...
    if (!(object[@"control_event"] && ([object[@"control_event"] unsignedIntegerValue] & UIControlEventAllEvents))) {
        ZhugeDebug(@"must supply a valid UIControlEvents value for control_event");
        return nil;
    }
    UIControlEvents verifyEvent = object[@"verify_event"] ? [object[@"verify_event"] unsignedIntegerValue] : 0;
    return [[ZGUIControlBinding alloc] initWithEventName:eventName
                                        onPath:path
...
- (void)execute
{
    ...
    if (!self.running) {
        ...
        [ZGSwizzler swizzleSelector:NSSelectorFromString(@"didMoveToWindow")
                            onClass:self.swizzleClass
                          withBlock:executeBlock
                              named:self.name];
        [ZGSwizzler swizzleSelector:NSSelectorFromString(@"didMoveToSuperview")
                            onClass:self.swizzleClass
                          withBlock:executeBlock
                              named:self.name];
        self.running = true;
...
- (void)stop
{
    if (self.running) {
        ...
        [ZGSwizzler unswizzleSelector:NSSelectorFromString(@"didMoveToWindow")
                            onClass:self.swizzleClass
                              named:self.name];
        [ZGSwizzler unswizzleSelector:NSSelectorFromString(@"didMoveToSuperview")
                            onClass:self.swizzleClass
                              named:self.name];
