...
@interface ZGObjectFilter : NSObject
...
@implementation ZGObjectSelector

+ (ZGObjectSelector *)objectSelectorWithString:(NSString *)string
{
    return [[ZGObjectSelector alloc] initWithString:string];
}
...
- (NSArray *)selectFromRoot:(id)root evaluatingFinalPredicate:(BOOL)finalPredicate
{
    ...
    if (root) {
        ...
        for (NSUInteger i = 0, n = [_filters count]; i < n; i++) {
            ZGObjectFilter *filter = _filters[i];
...
{
    if ([_filters count] > 0) {
        return NSClassFromString(((ZGObjectFilter *)_filters[[_filters count] - 1]).name);
    }
    return nil;
}
