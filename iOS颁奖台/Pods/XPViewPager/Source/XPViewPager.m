...
#import "XPViewPager.h"
#import <MJRefresh.h>
@interface XPViewPager ()
{
...
@implementation XPViewPager
{
    XP_VP_SelectedBlock _block;
    NSInteger _pageNum;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    for(NSInteger i = 0; i < _views.count; i++) {
        ...
        NSLog(@"titleLabel.frame:x:%lf width:%lf height:%lf", button.titleLabel.frame.origin.x, button.titleLabel.frame.size.width, button.titleLabel.frame.size.height);
        //创建菜单按钮右上角的小红点
...
- (void)didSelectedBlock:(XP_VP_SelectedBlock)block
{
    _block = block;
}
