...
@implementation ZGNSAttributedStringToNSDictionaryValueTransformer
...
- (id)transformedValue:(id)value
{
    if ([value isKindOfClass:[NSAttributedString class]]) {
        ...
        if (data) {
            ...
            ZhugeDebug(@"Failed to convert NSAttributedString to HTML: %@", error);
        }
    }
    ...
    if ([value isKindOfClass:[NSDictionary class]]) {
        ...
        if ([mimeType isEqualToString:@"text/html"] && dataString) {
            ...
            if (attributedString == nil) {
                ZhugeDebug(@"Failed to convert HTML to NSAttributed string: %@", error);
            }
