...
#import "ZGLog.h"
@implementation ZGUITableViewBinding
...
+ (ZGEventBinding *)bindingWithJSONObject:(NSDictionary *)object
{
    ...
    if (![eventName isKindOfClass:[NSString class]] || [eventName length] < 1 ) {
        ZhugeDebug(@"binding requires an event name");
        return nil;
    }

    Class tableDelegate = NSClassFromString(object[@"table_delegate"]);
    if (!tableDelegate) {
        ZhugeDebug(@"binding requires a table_delegate class");
        return nil;
    }

    return [[ZGUITableViewBinding alloc] initWithEventName:eventName
                                                  onPath:path
...
- (void)execute
{
    if (!self.running && self.swizzleClass != nil) {
        ...
        [ZGSwizzler swizzleSelector:@selector(tableView:didSelectRowAtIndexPath:)
...
{
    if (self.running && self.swizzleClass != nil) {
        [ZGSwizzler unswizzleSelector:@selector(tableView:didSelectRowAtIndexPath:)
                              onClass:self.swizzleClass
                                named:self.name];
        self.running = false;
    }
