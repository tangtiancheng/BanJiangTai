...
@implementation ZGUIImageToNSDictionaryValueTransformer
...
- (id)transformedValue:(id)value
{
    ...
    if ([value isKindOfClass:[UIImage class]]) {
        ...
        NSValueTransformer *sizeTransformer = [NSValueTransformer valueTransformerForName:NSStringFromClass([ZGCGSizeToNSDictionaryValueTransformer class])];
        NSValueTransformer *insetsTransformer = [NSValueTransformer valueTransformerForName:NSStringFromClass([ZGUIEdgeInsetsToNSDictionaryValueTransformer class])];
        ...
        for (UIImage *frame in images) {
            ...
            NSDictionary *imageDictionary = @{
                @"scale": @(image.scale),
                @"mime_type" : @"image/png",
                @"data": ((imageRep != nil) ? [imageRep zg_base64EncodedString] : [NSNull null])
            };
    ...
    if ([value isKindOfClass:[NSDictionary class]]) {
        ...
        for (NSDictionary *imageDictionary in imagesDictionary) {
            ...
            if (imageDictionary[@"url"]) {
                ...
                image = [UIImage imageWithData:[NSData zg_dataFromBase64String:imageDictionary[@"data"]] scale:fminf(1.0, [scale floatValue])];
            }
