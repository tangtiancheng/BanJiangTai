...
static void *ZGAllocBufferForObjCType(const char *objCType)
{
    void *buffer = NULL;
    ...
    if (result != 0) {
        ZhugeDebug(@"Error allocating aligned memory: %s", strerror(result));
...
@implementation NSInvocation (ZGHelpers)

- (void)zg_setArgument:(id)argumentValue atIndex:(NSUInteger)index
{
    ...
    else if ([argumentValue isKindOfClass:[NSValue class]])
    {
        ...
        void *buffer = ZGAllocBufferForObjCType([valueArgument objCType]);
...
- (void)zg_setArgumentsFromArray:(NSArray *)argumentArray
{
    ...
    for (NSUInteger i = 0; i < [argumentArray count]; ++i) {
        ...
        [self zg_setArgument:argumentArray[i] atIndex:argumentIndex];
    }
}
...
- (id)zg_returnValue
{
    ...
    void *buffer = ZGAllocBufferForObjCType(objCType);
