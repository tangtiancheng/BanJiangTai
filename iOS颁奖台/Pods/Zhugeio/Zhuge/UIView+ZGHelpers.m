...
#define ZG_FINGERPRINT_VERSION 1

@implementation UIView (ZGHelpers)

- (int)zg_fingerprintVersion
{
    return ZG_FINGERPRINT_VERSION;
}
...
{
    UIImage *image = [self zg_snapshotImage];
...
// Set by a userDefinedRuntimeAttr in the ZGTagNibs.rb script
...
- (NSString *)zg_controllerVariable
{
    NSString *result = nil;
    if ([self isKindOfClass:[UIControl class]]) {
...
static NSString* zg_encryptHelper(id input)
{
...
- (NSString *)zg_varA
{
    return zg_encryptHelper([self mp_viewId]);
}

- (NSString *)zg_varB
{
    return zg_encryptHelper([self zg_controllerVariable]);
}

- (NSString *)zg_varC
{
    return zg_encryptHelper([self zg_imageFingerprint]);
}

- (NSArray *)zg_varSetD
{
    NSArray *targetActions = [self zg_targetActions];
    NSMutableArray *encryptedActions = [NSMutableArray array];
    for (NSUInteger i = 0 ; i < [targetActions count]; i++) {
        [encryptedActions addObject:zg_encryptHelper(targetActions[i])];
    }
...
- (NSString *)zg_varE
{
    return zg_encryptHelper([self zg_text]);
}
